...
{ 
    ...
    { 
        atomic_dec(&dev->open_counter); printk("[pr0gr4m-blkdev] device was closed\n"); 
    } 
}   

static int dev_ioctl(struct block_device *bd, fmode_t mode, unsigned int cmd, unsigned long arg) 
{ 
    return -ENOTTY; 
}  
...
static int dev_compat_ioctl(struct block_device *bd, fmode_t mode, unsigned int cmd, unsigned long arg) 
{ return -ENOTTY; } 
...
{ 
    if (dev->data) 
    { 
        kfree(dev->data); dev->data = NULL; dev->capacity = 0; 
    } 
}   

static int blkdev_add_device(void)
{ 
    int ret = SUCCESS; struct gendisk *disk; 
    ...
    queue = blk_mq_init_sq_queue(&dev->tag_set, &mq_ops, 128, BLK_MQ_F_SHOULD_MERGE); 
    ...
    disk->flags |= GENHD_FL_REMOVABLE;  
     ...
     set_capacity(disk, dev->capacity); dev->gdisk = disk;   
     add_disk(disk); 
