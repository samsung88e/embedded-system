...
#define GPIO_alv2_OUT (162)

//LED is connected to this GPIO
#define GPIO_alv5_IN  (165)

//GPIO_alv5_IN value toggle
...
static irqreturn_t gpio_irq_handler(int irq,void *dev_id) 
{
  ...
  gpio_set_value(GPIO_alv2_OUT, led_toggle);                      // toggle the GPIO_alv2_OUT
  pr_info("Interrupt Occurred : GPIO_alv2_OUT : %d ",gpio_get_value(GPIO_alv2_OUT));
                ...
                char __user *buf, size_t len, loff_t *off)
{
  ...
  //reading GPIO value
  gpio_state = gpio_get_value(GPIO_alv2_OUT);
  ...
  pr_info("Read function : GPIO_alv2 = %d \n", gpio_state);
                ...
                const char __user *buf, size_t len, loff_t *off)
{
  ...
  pr_info("Write Function : GPIO_alv2 Set = %c\n", rec_buf[0]);
  
  if (rec_buf[0]=='1') {
    //set the GPIO value to HIGH
    gpio_set_value(GPIO_alv2_OUT, 1);
  } else if (rec_buf[0]=='0') {
    //set the GPIO value to LOW
    gpio_set_value(GPIO_alv2_OUT, 0);
  } else {
...
static int __init etx_driver_init(void)
{
  ...
  if(gpio_is_valid(GPIO_alv2_OUT) == false){
    pr_err("GPIO %d is not valid\n", GPIO_alv2_OUT);
    goto r_device;
  }
  
  //Requesting the GPIO
  if(gpio_request(GPIO_alv2_OUT,"GPIO_alv2_OUT") < 0){
    pr_err("ERROR: GPIO %d request\n", GPIO_alv2_OUT);
    goto r_gpio_out;
  }
  
  //configure the GPIO as output
  gpio_direction_output(GPIO_alv2_OUT, 1);
  
  //Input GPIO configuratioin
  //Checking the GPIO is valid or not
  if(gpio_is_valid(GPIO_alv5_IN) == false){
    pr_err("GPIO %d is not valid\n", GPIO_alv5_IN);
    goto r_gpio_in;
  }
  
  //Requesting the GPIO
  if(gpio_request(GPIO_alv5_IN,"GPIO_alv5_IN") < 0){
    pr_err("ERROR: GPIO %d request\n", GPIO_alv5_IN);
    goto r_gpio_in;
  }
  
  //configure the GPIO as input
  gpio_direction_input(GPIO_alv5_IN);
  ...
  */ 
  ...
  if(gpio_set_debounce(GPIO_alv5_IN, 200) < 0){
  ...
  GPIO_irqNumber = gpio_to_irq(GPIO_alv5_IN);
...
r_gpio_in:
  gpio_free(GPIO_alv5_IN);
r_gpio_out:
  gpio_free(GPIO_alv2_OUT);
r_device:
