 ...
 *
 * Hyunwoo, Kim <teddy0530@coasia.com>
 *
...
// gpio read
int nx_gpio_ctrl_read(struct udevice *dev, int offset, void *buf, int size)
{
	int ret;
...
// gpio write
int nx_gpio_ctrl_write(struct udevice *dev, int offset, void *buf, int size)
{
	int ret;
	unsigned char *data = buf;
	struct nx_gpio_ctrl_platdata *plat = dev_get_platdata(dev);
